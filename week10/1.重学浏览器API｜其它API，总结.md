...
- `range` API 确实很强大，但是并不是说有了锤子就到处找钉子，看到就用，而是心里要有一根弦，一个标准，比如在需要处理大量的 DOM 操作时或者精细操作时就需要使用，但其实 80% 的情况下都用不到，操作只有很少数量的 DOM 页面的改变实际上没必要使用。当然还有一个标准，你可以去查看 performance 的确定是否需要。
