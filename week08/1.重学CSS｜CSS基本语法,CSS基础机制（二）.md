...
- <简单选择器><简单选择器><简单选择器>
  - 无缝的写在一起（无空格和其他字符），因为「简单选择器」除了 tag 之外在选择器语法的开始都有一个特殊字符所以不会产生冲突。
  - 是「且、与」的关系，也就是必须同时满足这几种选择器的条件
- `*` 或者 `div` 必须写在最前面
  - 这是一个要求，`div` 是指 tag 选择器

#### 复杂选择器

复杂选择器是将复合选择器用一些操作符（operator）来进行连接。

- <复合选择器><sp\><复合选择器>
  - 子孙选择器
- <复合选择器> ">" <复合选择器>
  - 直接子元素选择器
  - 和子孙选择器类似，在实现的时候可以借用逻辑写在一起
- <复合选择器> "~" <复合选择器>
  - Sibling 相关
- <复合选择器> "+" <复合选择器>
  - Sibling 相关
...
- inline-style 比这些优先级都高，所以将选择器分为四级——行内、ID、class、标签
...
- `transform` 为什么这么复杂，有矩阵这些东西，这是因为 GPU 喜欢矩阵，做矩阵比较快，所以图形学中大量都是矩阵，拿坐标点和 `transform` 做计算，`transform` 本身就是一个矩阵，在做渲染之前会用现有坐标和 `transform` 一乘获得一个新的坐标。
...
- `:nth-child()`
  - **`:nth-child(an+b)`** 这个 [CSS 伪类](https://developer.mozilla.org/en-US/docs/CSS/Pseudo-classes)首先找到所有当前元素的兄弟元素，然后按照位置先后顺序从1开始排序，选择的结果为CSS伪类:nth-child括号中表达式（an+b）匹配到的元素集合（n=0，1，2，3...）。
  - 看自己是其父元素的哪个孩子
  - 注意所有简单选择器都是针对当前元素的
- `:nth-last-child()`
  - 这是浏览器工程师痛恨无比的伪类选择器
...
剩下的一些伪类选择器就是和特定的元素相绑定的，比如与 `input` 想绑定的 `:checked` ，这些伪类选择器你可以去学习对应的元素的时候就一起学习了。 
...
`::before` `::after` 我们可以使用它们来做一些额外的事情，比如你的内容没有语意，像是一朵花就可以使用这个来塞入进去。

这两个元素在网页中很重要，可以将「语意」和「表现」进行分离。它们也有产生新的盒的作用，元素不一定跟它的盒子是等同的概念。

> 不过呢，我觉得 html 中肯定有一些元素是用于专门做表现用的。
