...
- Identifier 分为两部分，作为变量名的部分，比如 `a` 和 `厉害` （通过 `var` `let` `const` 关键字生命的词），和用作属性名的部分 `write` 
  ...
  - 变量名是不能使用关键字来命名的，而属性名是可以的。
    ...
    - `dom` 元素的 `class` 属性之所以在 JS 中调用时是使用 `className` 来作为属性名，是因为 V3 版本不支持属性名使用关键字。并且现在当你调用某个变量的 `class` 属性时也不会报错，因为 JS 引入了新的词法解析基础设施。
...
- 支持 10、2、8、16 四种进制的表示法
  - 这在以前是不支持的，所以我们需要使用 `parstInt('10', 8)` 来转换
- 其中十进制有 4 种写法
...
- 这里写成 `97 .` 而不是 `97.` 的原因是解析器会将 `.` 认为是 `97` 的一部分，将其作为一个数字整体，而不是作为读取属性值的操作。
...
- GB 国标，GB 和 Unicode 的字符编码完全不同，但是兼容 ASCII
- BIG5 繁体中文
...
- `` 字符串模板
  ...
  - 这个字符串模板在引擎中分为三部分：
    ...
    - `sentance ` JS 表达式
    - `}" ` 
  - 不过我们作为使用者（用户）而言不需要知道引擎是怎么去转换的，但需要注意这使得规范上**顶级输入元素**有四个（除此之外也因为正则与 `\` 除号的冲突也单独做了区分）。不过呢，这对于使用者而言没有什么问题，而是对于实现的人而言会费很大的经。
