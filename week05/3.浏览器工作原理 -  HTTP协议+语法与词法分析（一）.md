...
- 当我们通过 `node client.js` 运行这个客户端后，连接成功，但是会返回 400，因为服务端不认识 `world!\r\m` 字符，所以我们需要更改请求文本
...
- `headers` ，必有 `Content-Type` 和 `Content-Length` 而 `Content-Type` 在这里使用最简单的用于表单提交的 `application/x-www-form-urlencoded`
  - 实际上 `Content-Type` 有很多中格式，比如当你要提交的表单中有文件的时候就需要使用 `multipart/form-data` 格式，提交 `XML` 时的 `text/xml` 和提交 `json` 的 `applicatioin/json` 
...
- `空行` 
- `body`
  - `body` 的 encode 比较复杂，node 的 `Transfer-Encoding` 一般会选用 `chunked` ，`chunked encoding` 的每一块是先由单独一行的整数来表示后面字符的数量，接着是这个数量的字符，然后又是单独一行的字符数量和字符，一直到 0 为止。
