...
出现的原因是大家想让 Promise 当普通函数去调用，用一种特殊的语法去等待 Promise 的改变之前中断函数内部的执行，然后等状态发生变化之后再执行之后的代码。
...
- 执行 `g()` 之后会返回一个 `Iterator` 的迭代器，然后调用其 `next()` 方法会返回一个带有 `value` 和 `done` 的对象，从这个对象中我们可以获取函数内返回的值和判断函数是否执行完毕。
