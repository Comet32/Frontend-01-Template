...
我们最终是希望 `publish-tool` 拿到 `token` ，对发布用户进行鉴权，然后再决定是否进行发布。（而我们先子是先发布了再走用户验证的流程）
...
- 发布系统用的是 nodejs 原声的东西，使用的是古典的回调处理，没有 promise，整个代码也比较乱，没有进行整理，这个可以自行改善整理。
